...
export function formatEmailBodyToHtml(text: string): string {
  ...
  const formatted = text
    ...
    // Links [text](url)
    .replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" target="_blank" style="color: #043084; font-weight: 700; text-decoration: underline;">$1</a>')
    // Plain URLs (if not inside href)
    .replace(/(https?:\/\/[^\s<]+)/g, (url) => {
      if (url.includes('href=')) return url;
      return `<a href="${url}" target="_blank" style="color: #043084; font-weight: 700; text-decoration: underline;">${url}</a>`;
    });
  ...
  const htmlBlocks = blocks.map((block) => {
    ...
    if (trimmed.includes("👉")) {
      return `<div style="background-color: #F8FAFC; border: 1.5px solid #E2E8F0; border-radius: 12px; padding: 14px 16px; margin: 16px 0; color: #043084; font-weight: 600;">${trimmed.replace(/\n/g, "<br/>")}</div>`;
    }
