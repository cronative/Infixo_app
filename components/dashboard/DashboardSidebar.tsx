...
export function DashboardSidebar() {
  ...
  return (
    <aside className="hidden w-64 shrink-0 flex-col border-r border-[#E9E3F5] bg-white px-4 py-5 lg:flex h-full overflow-y-auto">
      <div className="px-2">
        <Logo size="sm" />
      </div>

      {/* Creator Profile Card Banner (Solid Logo Brand Purple #651FFF) */}
      <div className="mt-5 rounded-2xl bg-[#651FFF] p-3.5 text-white shadow-md shadow-purple-600/25">
        <div className="flex items-center gap-2.5">
          <CreatorAvatar
            src={profile.photoDataUrl}
            name={profile.displayName || profile.email || "Creator"}
            className="h-10 w-10 rounded-full border-2 border-white/40 shadow-inner"
        ...
        {SIDEBAR_NAV.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 rounded-2xl px-3 py-2.5 text-xs font-extrabold transition-all ${
                active
      ...
      <div className="pt-4 border-t border-slate-100 space-y-3.5">
        ...
        <button
          ...
          className="flex w-full items-center gap-3 rounded-2xl px-3 py-2 text-xs font-extrabold text-rose-600 hover:bg-rose-50 transition-colors cursor-pointer"
        >
