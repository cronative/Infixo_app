      ...
      <div className="min-h-dvh flex flex-col items-center justify-center bg-[#FAF8FA] px-4 py-12 text-center text-[#17131A]">
        <div className="max-w-md space-y-6">
          ...
          <div className="rounded-3xl border border-[#ECE8EB] bg-white p-8 space-y-4 shadow-2xs">
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[#F7EDF3] text-[#600a0f]">
            ...
            <button
              onClick={() => router.push("/")}
              className="w-full py-2.5 rounded-xl bg-[#600a0f] text-white text-xs font-semibold hover:bg-[#6D3254] transition-colors"
            >
              Back to Home
            </button>
          </div>
      ...
      <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-[#ECE8EB] px-4 sm:px-8 py-3">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <Logo />
            <span className="text-xs font-bold text-[#600a0f] bg-[#F7EDF3] px-2 py-0.5 rounded-md">
              Media Kit
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl bg-[#600a0f] hover:bg-[#6D3254] px-4 py-1.5 text-xs font-semibold text-white transition-colors cursor-pointer shadow-2xs"
            >
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 pt-8 space-y-6 text-left">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs sm:text-sm font-semibold text-[#600a0f]">
            ...
            <div className="rounded-2xl border border-[#ECE8EB] bg-[#FAF8FA] p-4 text-center sm:text-right shrink-0 space-y-1">
              ...
              <p className="font-display text-3xl font-bold text-[#600a0f]">
                ...
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-1.5">
                      <span className="text-[11px] font-bold text-[#600a0f] bg-[#F7EDF3] px-2 py-0.5 rounded-lg">
                        {pkg.platform}
                      </span>
                    ...
                    <div>
                      ...
                      <div className="flex items-baseline gap-2 mt-1">
                        <span className="font-display text-lg font-bold text-[#600a0f]">
                          {pkg.price}
                        </span>
                  ...
                  <button
                    type="button"
                    onClick={() => setIsInquiryOpen(true)}
                    className="w-full py-2 rounded-xl bg-[#600a0f] hover:bg-[#6D3254] text-white text-xs font-semibold transition-colors cursor-pointer"
                  >
                    Request Booking
                  </button>
                </div>
        ...
        {team.members && team.members.length > 0 && (
          <section className="space-y-3.5">
            <div className="px-1">
              <h2 className="font-display text-base sm:text-lg font-bold text-[#17131A] flex items-center gap-2">
                <Users className="h-4 w-4 text-[#600a0f]" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              {team.members.map((m) => (
                <div key={m.id} className="rounded-2xl border border-[#ECE8EB] bg-white p-3.5 flex items-center gap-3 shadow-2xs">
                  <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#600a0f] text-white font-extrabold text-xs shrink-0 ring-2 ring-[#F7EDF3]">
                    {getInitials(m.name)}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-xs text-[#17131A] truncate">{m.name}</p>
                    <p className="text-[11px] font-semibold text-[#600a0f] truncate">{m.role}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="rounded-3xl border border-[#ECE8EB] bg-[#F7EDF3]/50 p-6 sm:p-8 text-center space-y-3">
          ...
          <div className="pt-2">
            <button
              type="button"
              onClick={() => setIsInquiryOpen(true)}
              className="inline-flex items-center gap-2 rounded-xl bg-[#600a0f] hover:bg-[#6D3254] px-6 py-2.5 text-xs font-semibold text-white transition-colors cursor-pointer shadow-2xs"
            >
