    ...
    <div className="space-y-6 text-left">
        ...
        {team && (
          <div className="flex items-center gap-2.5 shrink-0">
            <button
              ...
              className="tap-scale flex items-center gap-1.5 rounded-xl border border-[#ECE8EB] bg-white hover:bg-[#FAF8FA] px-3.5 py-2 text-xs font-semibold text-[#17131A] transition-colors cursor-pointer"
            >
            ...
            <button
              ...
              className="tap-scale flex items-center gap-1.5 rounded-xl bg-[#803D63] hover:bg-[#6F3456] text-white font-semibold text-xs py-2 px-4 transition-colors cursor-pointer shadow-2xs"
            >
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {/* Team Info Card */}
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 rounded-2xl border border-[#ECE8EB] bg-white p-5 shadow-2xs">
            <div className="flex items-center gap-3.5">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#803D63] text-white font-extrabold text-sm shadow-sm ring-2 ring-[#F7EDF3] shrink-0">
                {getInitials(team.teamName)}
              </div>
              <div>
                <h2 className="font-display text-base font-bold text-[#17131A]">{team.teamName}</h2>
          ...
          {(!team.members || team.members.length === 0) ? (
            <div className="rounded-2xl border border-dashed border-[#ECE8EB] bg-white p-8 text-center space-y-2">
              <p className="font-bold text-xs text-[#17131A]">No team members added yet</p>
              <p className="text-[11px] text-[#6F6872]">Click &quot;Add Member&quot; to showcase your collaborators.</p>
            </div>
          ) : (
              ...
              {team.members.map((member) => (
                <div
                  ...
                  className={`relative flex flex-col justify-between rounded-2xl border p-4 bg-white shadow-2xs transition-all ${
                    member.isActive ? "border-[#ECE8EB]" : "border-slate-200 opacity-60 bg-slate-50"
                  }`}
                >
                  <div className="flex items-start gap-3">
                    <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#803D63] text-white font-extrabold text-xs shadow-sm shrink-0 ring-2 ring-[#F7EDF3]">
                      {getInitials(member.name)}
                    </div>

                    <div className="min-w-0 flex-1">
                      <h3 className="truncate font-bold text-xs text-[#17131A]">{member.name}</h3>
      ...
      >
        <form onSubmit={handleSaveTeam} className="flex flex-col flex-1 min-h-0">
          <ModalBody className="p-5 sm:p-6 space-y-4 text-left">
            ...
            <div className="flex items-center gap-3.5 p-3 rounded-2xl bg-[#FAF8FA] border border-[#ECE8EB]">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#803D63] text-white font-extrabold text-sm shadow-sm ring-2 ring-[#F7EDF3] shrink-0">
                {getInitials(teamName || "Team")}
              </div>
              <div className="min-w-0">
                <p className="font-bold text-xs text-[#17131A] truncate">{teamName.trim() || "Team Name"}</p>
                <span className="inline-block text-[10px] font-semibold text-[#803D63] bg-[#F7EDF3] px-1.5 py-0.5 rounded-md">Creator Team</span>
              </div>
            </div>
      ...
      >
        <form onSubmit={handleSaveMember} className="flex flex-col flex-1 min-h-0">
          <ModalBody className="p-5 sm:p-6 space-y-4 text-left">
            ...
            <div className="flex items-center gap-3.5 p-3 rounded-2xl bg-[#FAF8FA] border border-[#ECE8EB]">
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#803D63] text-white font-extrabold text-sm shadow-sm ring-2 ring-[#F7EDF3] shrink-0">
                {getInitials(memberName || "Member")}
              </div>
              <div className="min-w-0">
                <p className="font-bold text-xs text-[#17131A] truncate">{memberName.trim() || "Member Name"}</p>
            ...
            <div className="space-y-3 pt-2 border-t border-[#ECE8EB]">
              <div>
                <p className="text-xs font-bold text-[#17131A]">Social Profiles (Optional)</p>
                <p className="text-[11px] text-[#6F6872] mt-0.5">Enter username only — full profile links are generated automatically.</p>
              </div>

              <div>
                <label className="block text-[11px] font-semibold text-[#6F6872] mb-1">
                  Instagram Username
                </label>
                <div className="flex h-10 items-center rounded-xl border border-[#ECE8EB] bg-[#FAF8FA] px-3 transition-colors focus-within:border-[#803D63] focus-within:bg-white">
                  ...
                  <input
                    ...
                    className="h-full w-full min-w-0 flex-1 bg-transparent text-xs font-semibold text-[#17131A] placeholder:text-[#6F6872]/40 outline-none"
                  />
                </div>
                <p className="mt-1 text-[10.5px] font-medium text-[#6F6872]/80 truncate">
