...
import AllSeriesClient from "./AllSeriesClient";

interface PageProps {
  params: Promise<{ username: string }>;
}

interface CreatorSeriesSeoRow extends RowDataPacket {
  username: string;
...
function parseVisibility(value?: string | null): { showSeries?: boolean; showInSearchEngines?: boolean } | null {
  if (!value) return null;
  try {
...
function isTrialPrivate(creator?: CreatorSeriesSeoRow | null) {
...
async function getCreatorSeriesSeo(username: string) {
  if (!username || username === "demo_creator") return null;

  const [rows] = await db.query<CreatorSeriesSeoRow[]>(
    `SELECT
       c.username,
       c.display_name,
       c.bio,
     ...
     LEFT JOIN series s ON s.creator_id = c.id
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `${SITE_URL}/${username}/series`;

  try {
    const creator = await getCreatorSeriesSeo(username);
    const visibility = parseVisibility(creator?.settings_visibility || creator?.visibility_settings);
    const shouldIndex = Boolean(creator) && !isTrialPrivate(creator) && visibility?.showSeries !== false && visibility?.showInSearchEngines !== false;

    if (!shouldIndex) {
      return {
        title: `@${username || "creator"} Series | Inflixo`,
        description: "This creator series page is not publicly available right now.",
    ...
    const seriesCount = Number(creator?.series_count || 0);
    const description =
      creator?.bio?.trim() ||
      `Explore ${displayName}'s ${seriesCount || ""} video series and playlists on Inflixo.`.replace("  ", " ");
    const image = creator?.photo_url || `${SITE_URL}/logo-square.png`;

    return {
      title: `${displayName}'s Series & Playlists | Inflixo`,
      description,
      alternates: { canonical: canonicalUrl },
      openGraph: {
        title: `${displayName}'s Series & Playlists`,
        description,
        url: canonicalUrl,
        siteName: "Inflixo",
        type: "website",
        images: [{ url: image, width: 1200, height: 630, alt: `${displayName} series on Inflixo` }],
      },
      twitter: {
    ...
    return {
      title: `@${username || "creator"} Series | Inflixo`,
      description: "Explore this creator's series and playlists on Inflixo.",
      alternates: { canonical: canonicalUrl },
    };
  }
}
