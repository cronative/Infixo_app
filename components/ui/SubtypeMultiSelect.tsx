  ...
  return (
    <div className="w-full space-y-2.5">
      <div className="flex flex-wrap items-center justify-between gap-1.5">
        ...
        <span
          className={`text-[11px] font-bold px-2 py-0.5 rounded-full border transition-all shrink-0 ${selectedSubtypes.length >= max
          ...
          return (
            <button
              ...
              className={`tap-scale flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold transition-all cursor-pointer ${isSelected
                ? "bg-[#600a0f] text-white shadow-sm ring-2 ring-purple-400/30"
                : isMaxReached
                  ? "opacity-40 cursor-not-allowed bg-white border border-slate-200 text-slate-400"
                  : "bg-white border border-slate-200 text-slate-700 hover:border-purple-300 hover:bg-purple-50/50 hover:text-[#600a0f]"
                }`}
            >
              {isSelected ? (
                <>
