  ...
  return (
    <div className="w-full space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-1.5">
        ...
        <span className="bg-[#600a0f]/[0.09] text-[#600a0f] border border-[#600a0f]/20 text-xs font-semibold px-2.5 py-0.5 rounded-full shrink-0">
            ...
            <button
              ...
              className={`tap-scale inline-flex items-center gap-1.5 text-xs py-1 px-3 rounded-full transition-all cursor-pointer ${isSelected
                ...
                : isMaxReached
                  ...
                  : "bg-white border border-[#E7E3DC] text-[#54514D] hover:border-[#600a0f]/30"
                }`}
            >
              <span className="text-xs shrink-0">{item.emoji}</span>
              <span className="truncate">{item.category}</span>
              {isSelected && <Check className="h-3 w-3 stroke-[3] text-[#600a0f] shrink-0" />}
            </button>
          );
        })}
      </div>
      ...
      {isOtherSelected && (
        <div className="animate-fade-in space-y-1.5 rounded-2xl border border-indigo-200 bg-indigo-50/60 p-3.5">
          ...
          <input
            ...
            className="w-full rounded-xl border border-indigo-200 bg-white px-3.5 py-2 text-xs font-bold text-slate-900 placeholder:text-slate-400 focus:border-[#600a0f] focus:outline-none focus:ring-2 focus:ring-indigo-200"
          />
