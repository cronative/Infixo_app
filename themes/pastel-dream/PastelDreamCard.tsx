  ...
  return (
    ...
    >
      <div className="flex flex-col items-center text-center">
        ...
        <p className={`mt-4 ${full ? "text-2xl" : "text-lg"} font-extrabold text-[#043084]`}>
          {profile.displayName || "Your name"}
        </p>
        <p className="text-sm text-[#043084]/55">
          @{profile.username || "username"} {profile.category ? `· ${profile.category}` : ""}
        </p>
        <p className="mt-3 max-w-sm text-sm leading-relaxed text-[#043084]/75">{profile.bio}</p>
        ...
        <div className="mt-4 flex items-center gap-1.5 rounded-full bg-white/70 px-3.5 py-1.5 shadow-sm">
          ...
          <span className="text-xs font-bold text-[#043084]">{formatCount(totalAudience)} total fanbase</span>
        </div>
      </div>
          ...
          {series.slice(0, full ? undefined : 2).map((s) => (
            <div key={s.id} className="flex items-center gap-3 rounded-2xl bg-white/70 px-3 py-2.5 shadow-sm">
              ...
              <div className="min-w-0">
                <p className="truncate text-sm font-bold text-[#043084]">{s.title}</p>
                <p className="text-xs text-[#043084]/50">{s.seasons.length} season{s.seasons.length !== 1 ? "s" : ""}</p>
              </div>
            </div>
          ))}
        </div>
...
function StatCell({ icon, value }: { icon: React.ReactNode; value: number }) {
  return (
    <div className="flex flex-col items-center gap-1 rounded-2xl bg-white/70 py-3 text-[#043084] shadow-sm">
      {icon}
