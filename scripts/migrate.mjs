...
const layoutSql = await readFile(new URL("../db/migrations/003_public_profile_layout.sql", import.meta.url), "utf8");

const targets = [
  {
    name: "inflixo_db (Development / Local)",
  ...
  {
    name: "inflixo_prod (Production)",
    host: process.env.PROD_MYSQL_HOST || process.env.MYSQL_HOST,
    user: process.env.PROD_MYSQL_USER || "inflixo_prod_user",
    password: process.env.PROD_MYSQL_PASSWORD || "InflixoProd@2026!",
    database: process.env.PROD_MYSQL_DATABASE || "inflixo_prod",
    port: Number(process.env.PROD_MYSQL_PORT) || Number(process.env.MYSQL_PORT) || 3306,
  },
];
...
async function migrateDatabase(target) {
  if (!target.host || !target.user || !target.database) {
    console.warn(`⚠️  Skipping ${target.name}: incomplete connection settings.`);
    return;
  }
...
console.log("\n🎉 All database migrations (inflixo_db & inflixo_prod) completed successfully.");
