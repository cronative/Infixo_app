    ...
    <OnboardingLayout step="series">
      ...
      <div className="mt-4 space-y-3.5 text-left">
                  ...
                  return (
                    <button
                      ...
                      className={`tap-scale flex items-center justify-center gap-1.5 rounded-xl border px-2.5 py-1.5 text-xs font-semibold transition-all cursor-pointer ${isSelected
                        ? "border-[#043084] bg-[#043084]/[0.08] text-[#043084] font-bold"
                        : "border-[#E7E3DC] bg-white text-[#54514D] hover:bg-surface-soft"
                        }`}
                    >
                      {p === "YouTube" && <YoutubeIcon className={`h-3.5 w-3.5 ${isSelected ? "text-[#043084]" : "text-red-500"}`} />}
                      {p === "Instagram" && <InstagramIcon className={`h-3.5 w-3.5 ${isSelected ? "text-[#043084]" : "text-pink-500"}`} />}
                      {p === "Facebook" && <FacebookIcon className={`h-3.5 w-3.5 ${isSelected ? "text-[#043084]" : "text-blue-600"}`} />}
                      {p === "Other" && <Globe className={`h-3.5 w-3.5 ${isSelected ? "text-[#043084]" : "text-[#797570]"}`} />}
                      <span>{p}</span>
                    </button>
                  );
                })}
        ...
        <div className="rounded-xl border border-[#E7E3DC] bg-white p-3 text-xs text-[#54514D] flex items-start gap-2">
          <Sparkles className="h-3.5 w-3.5 text-[#043084] shrink-0 mt-0.5" />
      ...
      <div className="pt-3.5 border-t border-[#E7E3DC] mt-5 sm:mt-6 flex flex-col-reverse sm:flex-row items-center justify-between gap-3">
        ...
        <Button
          ...
          className="w-full sm:flex-1 sm:max-w-xs h-10 bg-[#043084] hover:bg-brand-hover text-white font-bold text-xs sm:text-sm rounded-xl cursor-pointer shadow-xs"
        >
          Save &amp; Next →
        </Button>
      </div>
